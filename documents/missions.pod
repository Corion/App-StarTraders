...
It may be unclear at first, but the "player character" is in fact
the ship. Its crew are accessories, much like other equipment.
...
=head2 Cause and effect

If something is forbidden, show why forbidding this is bad. For example,
if technology or weapons are forbidden, show how people suffer from it
and how they would be better off with them.

If something is needed, show why it is needed. If water is needed,
show how people are worse off without the water and how they need it to
water their crops and how they (would) starve without it.

=head1 Missions

=head2 The overarching mission

Journey to the Center of the World (Universe/Galaxy) ?

=head2 Quest generator

L<https://gist.github.com/Ovid/0674ef8fe906f10cc8f2>

=head2 First mission arc

    Some tutorial trading missions

Even though the environment is basically a sandbox environment, 
the player needs something to do. A few immediate missions and a medium
goal need to be presented. Whether the player accepts these or goes
off on their own way is immaterial, but there needs to be more direction
at the start.

=head3 Some tutorial trading missions

Maybe a tutorial mission or some would be convenient for players fresh
to the environment. These missions should not be mandatory to keep replay
value high for experienced players.

=head2 Exit quest/plotline

These quests lure the player into exploring new terrain.
...
=head2 Follow the Dead Rabbit

Just like Follow the White Rabbit, except you don't have to run as much.
...
The player (character) can develop the technology to escape from
the current golden cage.

(Exit quest/plotline)

=item *

The player gets simply expelled by their community for transgressing
a (made-up after the fact) law.

(Exit quest/plotline)

=item *

The player gets expelled from the system by force of nature.

=back

=head2 Empathy With The Devil

"Have a "villain" that has reasons for their doing.
Make the player feel empathy."

=head3 Unfortunate Number Two

The hero is attacked by a pitiful person, and only after killing the NPC
it is found that the pitiful person was destined to be the saviour
of their planet (or whatever), but the player through their actions
prevented that unknowingly (as that prediction is made up post-factum).
All the preventions should be really minor actions, like buying up all
ore when the NCP really needed that ore, but was two hours late and a
credit short. This will either go completely unnoticed or give the player
something to think about.

Maybe helping that NPC should be an option, or maybe that NPC just is a
tragic figure, who can be avoided but not helped at all. Avoiding the NPC
just prolongs its (imaginary) suffering, as any player action is a potential
further nail in the NPCs cross/coffin. "Helping" the NPC will also need
to result in either acerbating the situation or simply result in the NPC
losing their "saviour" status and the player gaining that (unwanted by then)
status. Having that NPC a tragic figure might make killing him more
painful every time the player has to do so. The history why the paths
cross needs to be interesting enough so the player still reads it
out of curiosity, even after the 100th time.

=head3 Your predecessor (Gollum)

As an alternative to Unfortunate Number Two, meeting somebody (or rather,
finding their trails) who did the same things as the player, except
that these steps led to failure ultimately, might be an interesting thing.
It would maybe make the player question what consequences their actions
had, in the reflection of the actions of the one who went before them.
...
Also see Follow The Dead Rabbit.

=head3 The Teacher is an AI

As ship AIs are ubiquitous (and potentially can be transferred from
ship to ship), such an AI can go rouge, especially after it has
amassed enough weapons and/or a place to go to.
...
=head2 Automatic mission generation

Review L<http://larc.unt.edu/techreports/LARC-2011-02.pdf> , it contains
...
Question building parts

=back

=head1 Play style

The gameplay consists of three main play areas:
...
This is the navigation between star systems and planets. Things to do
are mostly moving between systems, and to scan for new pathways.

=item #

Ship boarding

This is a micro-management turn based strategy in the style of the old
X-Com games. This can be used to board ships and space stations and to
scavenge interesting information there.

=item #

Station trade

Trading items between ship and a planet or space station. Note that "money"
has little use, but energy should be a fairly universal commodity that
can be used in place of "money" to compare prices.

=back

=head1 Reflection of the Player

The player gets a view on his actions through several mirrors.

=over 4

=item *

The newscasts that comment on their behaviour and the implications
in the short term

=item *

The unfortunate predecessors
...
In any event, the "intro, four planets, finale" structure is
something we have used often for a few simple reasons:
...
Creating such a stash also is a way for the player to store away riches
or send them towards his home base. Of course, a certain ratio of such
stashes will run astray and get found by reincarnations of the player
in other game instances.

=head2 Connections

There could be jumpgates between systems but a player can later on also
deploy their own buoys. This is game-breaking akin a Wand of Digging.

=head2 The Cult

(taken from Shamus Young's review of Mass Effect)

The outpost of Zhu’s Hope feels a lot like a remix of the Mother quest from
Jade Empire. (Which itself borrowed a little from the Rakghoul plot in KOTOR.)
You enter a small community of odd people who are suspicious, unwelcoming, and
secretive. Their dialog doesn’t make a lot of sense and it sort of feels like
you’ve stumbled into a cult. Then after some screwing around you make it
underground to find everyone was being controlled by a huge monster. Then you
beat the monster and everyone is set free.
...
There are some characters or character archetypes the player can encounter.
If we have a party system (ship crew) for the ship, these characters can
join the ship crew.

=head2 The Swarm

This is an entity who consists of individuals of seemingly different
races. The Swarm can supply Brutes to help when boarding other ships.
Once The Swarm gets big enough, it will divide into two new swarms.
One of them may or may not remain with the crew.
