...
sub get_effects( $base, $effects, @active_effects ) {
    for my $effect ( @active_effects ) {
        $effect->apply( $base, $effects );
    }
};

sub get_effects_delta ( $tick_count, $base, @active_effects ) {
    get_effects( $base, \my %effects, @active_effects );

    \%effects
};

# These are the base stats of the player
# unadorned by effects
# player_body -> life_choices/adventure buffs/permanent effects, like levelling -> effects
my %player = (
    effects => \@active_effects, # this calculates the maximum
    current => undef,
);
...
my $active_effects= get_effects_delta(undef, {}, @active_effects);

sub apply_effects( $player, $effects_delta ) {
    ...
    for my $attr (keys %$effects_delta) {
        if( $effects_delta->{ $attr }) {
            $res{ $attr } = ($player->{$attr}|| 0) * ($effects_delta->{$attr}->{scale} || 1)+ ($effects_delta->{$attr}->{buff} || 0); 
        } else {
            $res{ $attr } = $player->{$attr}; 
        }
    }
    \%res
}

$player{ current } = apply_effects({}, $active_effects);
# we should strip out all attributes where max=current according to the rules
warn Dumper \$player{current};

push @active_effects, RPG::Effect->new(
        { name => 'dunce cap', base_attribute => 'intelligence', affected_attribute => 'intelligence', buff => -15, scale => 0 },
);

$active_effects= get_effects_delta(undef, {}, @active_effects);
warn Dumper $active_effects;
$player{ current } = apply_effects({}, $active_effects);
# we should strip out all attributes where max=current according to the rules
warn Dumper \$player{current};
